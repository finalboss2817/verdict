...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (sanitized.length < 10) {
      setError("Please provide more context for the objection.");
      return;
    }
    ...
    try {
      ...
      // Display the actual error message from the Gemini SDK for transparency
  ...
  if (isInitialLoading) return (
    <div className="min-h-screen bg-[#070707] flex items-center justify-center text-zinc-500">
    ...
    <div className="flex h-screen bg-[#070707] text-zinc-100 overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 lg:relative z-50 bg-zinc-950 border-r border-zinc-900 flex flex-col transition-all duration-500 ${isSidebarOpen ? 'translate-x-0 w-80' : '-translate-x-full w-80 lg:translate-x-0 lg:w-0 lg:opacity-0'}`}>
        ...
        <nav className="p-4 border-b border-zinc-900 space-y-1">
          <button onClick={() => { setActiveView('engine'); if(window.innerWidth < 1024) setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeView === 'engine' ? 'bg-zinc-900 text-zinc-100 shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}>
            <LayoutDashboard size={18} /> Engine
          </button>
          <button onClick={() => { setActiveView('protocol'); if(window.innerWidth < 1024) setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeView === 'protocol' ? 'bg-zinc-900 text-zinc-100 shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}>
        ...
        <div className="p-4 border-t border-zinc-900 bg-zinc-950 flex items-center justify-between">
          ...
          <button onClick={() => supabase.auth.signOut()} className="p-2 text-zinc-600 hover:text-rose-400 transition-colors"><LogOut size={16} /></button>
        </div>
      </aside>
        ...
        <header className="h-16 border-b border-zinc-900 flex items-center justify-between px-6 bg-[#070707]/95 backdrop-blur-md z-30">
          ...
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 rounded-full border border-zinc-800">
               ...
               <span className="text-[9px] font-black uppercase text-zinc-400">Lite Engine Ready</span>
             </div>
                ...
                <div className="space-y-12 animate-in fade-in slide-in-from-top-4 duration-700">
                  ...
                  <form onSubmit={handleSubmit} className="bg-zinc-900/10 border border-zinc-800/50 rounded-[40px] p-8 md:p-14 space-y-8 backdrop-blur-3xl shadow-2xl relative">
                    <div className="grid grid-cols-2 gap-4">
                      <button type="button" onClick={() => setFormData({...formData, mode: 'VOID'})} className={`p-6 rounded-3xl border flex flex-col items-center gap-3 transition-all duration-300 ${formData.mode === 'VOID' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-500 shadow-inner' : 'bg-zinc-950/50 border-zinc-800 text-zinc-600 hover:border-zinc-700'}`}>
                        <Zap size={24} /> 
                        <span className="font-black text-[10px] uppercase tracking-widest">VOID Mode</span>
                      </button>
                      <button type="button" onClick={() => setFormData({...formData, mode: 'NEXUS'})} className={`p-6 rounded-3xl border flex flex-col items-center gap-3 transition-all duration-300 ${formData.mode === 'NEXUS' ? 'bg-blue-500/10 border-blue-500/50 text-blue-500 shadow-inner' : 'bg-zinc-950/50 border-zinc-800 text-zinc-600 hover:border-zinc-700'}`}>
                        <Crosshair size={24} /> 
                        <span className="font-black text-[10px] uppercase tracking-widest">NEXUS Mode</span>
                      </button>
                    </div>

                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest ml-2">Human Objection</label>
                      <textarea required placeholder='"Budget is frozen until next fiscal..."' className="w-full bg-zinc-950/80 border border-zinc-800 rounded-3xl p-8 text-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-700 min-h-[160px] resize-none text-base placeholder:text-zinc-800 transition-all shadow-inner" value={formData.objection} onChange={(e) => setFormData({...formData, objection: e.target.value})} />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <select className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-[10px] text-zinc-400 font-bold uppercase tracking-widest focus:border-zinc-700 appearance-none" value={formData.ticketSize} onChange={(e) => setFormData({...formData, ticketSize: e.target.value})}>
                      ...
                      <select className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-[10px] text-zinc-400 font-bold uppercase tracking-widest focus:border-zinc-700 appearance-none" value={formData.stage} onChange={(e) => setFormData({...formData, stage: e.target.value})}>
                        <option>Discovery</option><option>Proposal</option><option>Hard Closing</option>
                      </select>
                    </div>

                    <div className="pt-4 space-y-6">
                      <button disabled={isLoading} className="w-full bg-zinc-100 hover:bg-white text-black font-black py-6 rounded-3xl flex items-center justify-center gap-4 transition-all disabled:opacity-50 text-xs tracking-[0.2em] uppercase active:scale-[0.98] shadow-2xl">
                        {isLoading ? <Loader2 size={24} className="animate-spin" /> : <><Send size={18} /> Decode Intent</>}
                      </button>
